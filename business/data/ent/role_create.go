...
func (rc *RoleCreate) AddAuthUsersRoleIDs(ids ...int) *RoleCreate {
	rc.mutation.AddAuthUsersRoleIDs(ids...)
	return rc
}

// AddAuthUsersRoles adds the "auth_users_roles" edges to the UsersRoles entity.
func (rc *RoleCreate) AddAuthUsersRoles(u ...*UsersRoles) *RoleCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return rc.AddAuthUsersRoleIDs(ids...)
}
...
func (rc *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.UsersIDs(); len(nodes) > 0 {
		...
		createE := &UsersRolesCreate{config: rc.config, mutation: newUsersRolesMutation(rc.config, OpCreate)}
		createE.defaults()
		_, specE := createE.createSpec()
		edge.Target.Fields = specE.Fields
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := rc.mutation.AuthUsersRolesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
